...
function moveFocus(i) {
    ...
    if (!(i == answer.length - 1) && event.keyCode !== 8) $input[i + 1].focus();
}
...
function reset() {
    ...
    for (let i = 0; i < 26; i++) {
        $keyboxReset[i].classList.remove('green');
        $keyboxReset[i].classList.remove('yellow');
        $keyboxReset[i].classList.remove('lightgrey');
    }
    t = 0;
